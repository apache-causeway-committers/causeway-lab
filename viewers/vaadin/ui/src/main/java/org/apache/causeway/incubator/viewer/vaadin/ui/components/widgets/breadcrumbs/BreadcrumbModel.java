 ...
 *  under the License.
 */
package org.apache.causeway.incubator.viewer.vaadin.ui.components.widgets.breadcrumbs;
...
public class BreadcrumbModel implements HasMetaModelContext, Serializable {
    ...
    public List<UiObjectVdn> getList() {
        List<UiObjectVdn> entityModels = _Lists.newArrayList();
        for (Bookmark bookmark : list) {
            UiObjectVdn entityModel = toEntityModel(bookmark);
    ...
    public UiObjectVdn getMostRecentlyVisited() {
        return mostRecentlyVisitedOidStr != null ? lookup(mostRecentlyVisitedOidStr) : null;
    }

    public void visited(final UiObjectVdn entityModel) {
    ...
    private String oidStrFrom(final UiObjectVdn entityModel) {
    ...
    private void addToStart(final String oidStr, final UiObjectVdn entityModel) {
        Bookmark bookmark = (Bookmark) entityModel.getOwnerBookmark();
    ...
    public UiObjectVdn lookup(final String oidStr) {
        if (oidStr == null) {
            return null;
        }
    ...
    public void remove(final UiObjectVdn entityModel) {
        Bookmark bookmark = (Bookmark) entityModel.getOwnerBookmark();
        final String oidStr = oidStrByBookmark.get(bookmark);
        if (oidStr != null) {
            remove(oidStr, bookmark);
    ...
    protected UiObjectVdn toEntityModel(final Bookmark bookmark) {
        return UiObjectVdn.ofBookmark(bookmark);
    }
